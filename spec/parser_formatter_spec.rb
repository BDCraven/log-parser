...
describe ParserFormatter do
  ...
  describe '#print_webpages' do
    it 'returns a string of results' do
      expect { subject.printwebpages(counted_webpage_views) }.to output(<<~HEREDOC).to_stdout
      /about/2 2 visits
      /contact 1 visits
  ...
  describe '#print_unique_webpages' do
    it 'returns a string of results' do
      expect { subject.printuniquewebpages(counted_webpage_views) }.to output(<<~HEREDOC).to_stdout
